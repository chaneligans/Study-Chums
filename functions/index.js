...
exports.sendRequestEmail = functions.database.ref('Applications/{uid}/Sent/{values}').onWrite((change, context) => {
  ...
  var recipient = change.after.forEach((snapshot) => {
    ...
    Promise.all([getRecipientName, getFirebaseEmail]).then(values => {
      // Send mail
      var mailOptions = {
